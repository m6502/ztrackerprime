...
- **Don't ask the user to test.** Run the binary yourself first and verify basic rendering. Default tool is `build/Program/zt --headless --script <file>` (see `docs/headless-script.md` and `.claude/skills/ztrackerprime/SKILL.md` → "Headless verification") — SDL dummy video driver, opens NO window, dumps PNGs of the in-memory surface, zero focus theft. **If the user says "headless", it MUST be the --headless path** — never silently fall back to AppleScript / `scripts/zt-screenshot.sh`. Those tools ARE fine when the user explicitly asks for a screenshot-driven smoke test, but always interrupt their screen, so reserve them for that case. The 2026-05-18 lesson: I kept using AppleScript activate after Esa asked for "headless", which hijacked his screen for minutes at a time. Only ask the user when something requires their interactive judgement or hardware they alone have (e.g. real MIDI device CC verification).
